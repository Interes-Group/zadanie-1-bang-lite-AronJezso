    ...
    public void play(ArrayList<Player> players, int player, Deck deck) {
        ...
        for (int currPlayer = 0; currPlayer < players.size(); currPlayer++) {
            if (currPlayer != player) {
                ...
                for (int v = 0; v < players.get(currPlayer).getHand().size(); v++) {
                    if ((players.get(currPlayer).getHand().get(v) instanceof Bang)) {
                        deck.addBin(players.get(currPlayer).getHand().get(v));
            ...
            if (!players.get(currPlayer).isAlive()) {
                ...
                deck.addAllBin(players.get(currPlayer).getHand());
                players.get(currPlayer).getHand().removeAll(players.get(currPlayer).getHand());
